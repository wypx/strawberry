...
type EndpointService struct {
	isPeer              bool
	isReady             bool
	groupListener       *VmUtilsNetwork.Listener
	groupPinger         *VmUtilsNetwork.Pinger
...
func CreateStubEndpoint(groupAddress string, groupPort int, domain, listenAddress string) (endpoint EndpointService, err error) {
	...
	listener, err := VmUtilsNetwork.CreateListener(groupAddress, groupPort, domain, listenInterface)
	if err != nil {
		return endpoint, err
	}
...
func CreatePeerEndpoint(groupAddress string, groupPort int, domain string) (endpoint EndpointService, err error) {
	pinger, err := VmUtilsNetwork.CreatePinger(groupAddress, groupPort, domain)
	if err != nil {
		return endpoint, err
	}
...
func (endpoint *EndpointService) recoverStubService() {
	...
	for endpoint.isRunning() {
		...
		pinger, err := VmUtilsNetwork.CreatePinger(endpoint.groupAddress, endpoint.groupPort, endpoint.domain)
