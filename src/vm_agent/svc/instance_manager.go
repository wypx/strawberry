...
func (manager *InstanceManager) Routine(c VmUtils.RoutineController) {
...
func (config *GuestConfig) Marshal(message VmUtils.Message) error {
	message.SetString(VmUtils.ParamKeyName, config.Name)
	message.SetString(VmUtils.ParamKeyInstance, config.ID)
	message.SetString(VmUtils.ParamKeyUser, config.User)
	message.SetString(VmUtils.ParamKeyGroup, config.Group)
	message.SetString(VmUtils.ParamKeySystem, config.System)
	message.SetUInt(VmUtils.ParamKeyCore, config.Cores)

	if config.AutoStart {
		message.SetUIntArray(VmUtils.ParamKeyOption, []uint64{1})
	} else {
		message.SetUIntArray(VmUtils.ParamKeyOption, []uint64{0})
	}
	if config.Created {
		message.SetBoolean(VmUtils.ParamKeyEnable, true)
	} else {
		message.SetBoolean(VmUtils.ParamKeyEnable, false)
		message.SetUInt(VmUtils.ParamKeyProgress, config.Progress)
	}

	if config.Running {
		message.SetUInt(VmUtils.ParamKeyStatus, InstanceStatusRunning)
	} else {
		message.SetUInt(VmUtils.ParamKeyStatus, InstanceStatusStopped)
	}

	message.SetUInt(VmUtils.ParamKeyMonitor, config.MonitorPort)
	message.SetString(VmUtils.ParamKeySecret, config.MonitorSecret)
	message.SetUInt(VmUtils.ParamKeyMemory, config.Memory)
	message.SetUIntArray(VmUtils.ParamKeyDisk, config.Disks)
	message.SetString(VmUtils.ParamKeyAddress, config.NetworkAddress)
	message.SetString(VmUtils.ParamKeyCreate, config.CreateTime)
	message.SetString(VmUtils.ParamKeyHardware, config.HardwareAddress)
	//QoS
	message.SetUInt(VmUtils.ParamKeyPriority, uint(config.CPUPriority))
	message.SetUIntArray(VmUtils.ParamKeyLimit, []uint64{config.ReadSpeed, config.WriteSpeed, config.ReadIOPS,
		config.WriteIOPS, config.ReceiveSpeed, config.SendSpeed})
	return nil
}

func (status *InstanceStatus) Marshal(message VmUtils.Message) error {
	if err := status.GuestConfig.Marshal(message); err != nil {
		return err
	}
	message.SetBoolean(VmUtils.ParamKeyMedia, status.MediaAttached)
	message.SetFloat(VmUtils.ParamKeyUsage, status.CpuUsages)
	message.SetUIntArray(VmUtils.ParamKeyAvailable, []uint64{status.AvailableMemory, status.AvailableDisk})
	message.SetUIntArray(VmUtils.ParamKeyIO,
