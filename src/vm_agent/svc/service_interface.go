...
type StorageModule interface {
	...
	ReadDiskImage(id VmUtils.SessionID, groupName, targetVol, sourceImage string, targetSize, imageSize uint64,
		mediaHost string, mediaPort uint, startChan chan error, progress chan uint, resultChan chan StorageResult)
	WriteDiskImage(id VmUtils.SessionID, groupName, targetVol, sourceImage, mediaHost string, mediaPort uint,
		startChan chan error, progress chan uint, resultChan chan StorageResult)
	ResizeVolume(id VmUtils.SessionID, groupName, targetVol string, targetSize uint64, respChan chan StorageResult)
	ShrinkVolume(id VmUtils.SessionID, groupName, targetVol string, respChan chan StorageResult)
	//snapshot
