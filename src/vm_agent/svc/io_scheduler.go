...
func CreateScheduler(poolName string, progressChan chan SchedulerUpdate, resultChan chan SchedulerResult, eventChan chan schedulerEvent) (scheduler *IOScheduler, err error) {
	...
	scheduler.runner = VmUtils.CreateSimpleRunner(scheduler.Routine)
...
func (scheduler *IOScheduler) Routine(c VmUtils.RoutineController) {
...
func (scheduler *IOScheduler) AddWriteTask(id VmUtils.SessionID, group, volume, path, image, host string, port uint) {
...
func (scheduler *IOScheduler) AddReadTask(id VmUtils.SessionID, group, volume, path, image string, targetSize, imgSize uint64, host string, port uint) {
...
func (scheduler *IOScheduler) AddResizeTask(id VmUtils.SessionID, group, volume, path string, size uint64) {
	scheduler.taskChan <- scheduleTask{Type: scheduleResizeDisk, ID: id, Group: group, Volume: volume, Path: path, TargetSize: size}
}

func (scheduler *IOScheduler) AddShrinkTask(id VmUtils.SessionID, group, volume, path string) {
...
func (scheduler *IOScheduler) handleWriteTask(id VmUtils.SessionID, group, volume, path, image, host string, port uint) (err error) {
	const (
		Protocol       = "https"
...
func (scheduler *IOScheduler) handleReadTask(id VmUtils.SessionID, group, volume, path, image string, imageSize, targetSize uint64, host string, port uint) (err error) {
	const (
		Protocol       = "https"
		Resource       = "disk_images"
		ChunkSize      = 1 << 10
...
func (scheduler *IOScheduler) handleResizeTask(id VmUtils.SessionID, group, volume, path string, size uint64) (err error) {
	defer func(e error) {
...
func (scheduler *IOScheduler) handleShrinkTask(id VmUtils.SessionID, group, volume, path string) (err error) {
	defer func(e error) {
