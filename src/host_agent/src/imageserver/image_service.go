...
func (service *ImageService) OnMessageReceived(msg vm_utils.Message) {
	...
	case vm_utils.DiskImageUpdatedEvent:
...
func (service *ImageService) OnServiceConnected(nodeName string, t vm_utils.ServiceType, address string) {
	switch t {
	case vm_utils.ServiceTypeCore:
		event, _ := vm_utils.CreateJsonMessage(vm_utils.ImageServerAvailableEvent)
		event.SetString(vm_utils.ParamKeyName, service.GetName())
		event.SetString(vm_utils.ParamKeyHost, service.httpModule.GetHost())
		event.SetUInt(vm_utils.ParamKeyPort, uint(service.httpModule.GetPort()))
...
func (service *ImageService) OnServiceDisconnected(name string, t vm_utils.ServiceType, gracefully bool) {
