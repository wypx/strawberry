...
func (executor *CreateGuestExecutor) Execute(id vm_utils.SessionID, request vm_utils.Message,
	incoming chan vm_utils.Message, terminate chan bool) (err error) {
	resp, _ := vm_utils.CreateJsonMessage(vm_utils.CreateGuestResponse)
	...
	if guestName, err = request.GetString(vm_utils.ParamKeyName); err != nil {
		return err
	}
	...
	if config.Pool, err = request.GetString(vm_utils.ParamKeyPool); err != nil {
		return err
	}

	if config.Cores, err = request.GetUInt(vm_utils.ParamKeyCore); err != nil {
		return err
	}
	if config.Memory, err = request.GetUInt(vm_utils.ParamKeyMemory); err != nil {
		return err
	}
	if config.Disks, err = request.GetUIntArray(vm_utils.ParamKeyDisk); err != nil {
		return err
	}
	...
	if config.AutoStart, err = request.GetBoolean(vm_utils.ParamKeyOption); err != nil {
		return err
	}
	...
	if templateID, err = request.GetString(vm_utils.ParamKeyTemplate); err != nil {
		...
		currentAdmin, _ = request.GetString(vm_utils.ParamKeyAdmin)
		if "" == currentAdmin {
			request.SetString(vm_utils.ParamKeyAdmin, t.Admin)
		}
		var options []uint64
		...
		request.SetUIntArray(vm_utils.ParamKeyTemplate, options)
	}
	//Security policy
	{
		var policyID string
		if policyID, err = request.GetString(vm_utils.ParamKeyPolicy); nil == err && "" != policyID {
			var respChan = make(chan modules.ResourceResult, 1)
			...
			request.SetBoolean(vm_utils.ParamKeyAction, policy.Accept)
			...
			request.SetUIntArray(vm_utils.ParamKeyPolicy, policyParameters)
		}
	}

	//QoS
	{
		priorityValue, _ := request.GetUInt(vm_utils.ParamKeyPriority)
		config.CPUPriority = modules.PriorityEnum(priorityValue)
		if limitParameters, err := request.GetUIntArray(vm_utils.ParamKeyLimit); err == nil {
			const (
	...
	request.SetString(vm_utils.ParamKeyName, config.Name)
	...
	if imageID, err := request.GetString(vm_utils.ParamKeyImage); err == nil {
		...
		{
			query, _ := vm_utils.CreateJsonMessage(vm_utils.GetDiskImageRequest)
			query.SetFromSession(id)
			query.SetString(vm_utils.ParamKeyImage, imageID)
			...
			case queryResp := <-incoming:
				...
				imageName, _ = queryResp.GetString(vm_utils.ParamKeyName)
				imageSize, _ = queryResp.GetUInt(vm_utils.ParamKeySize)
				imageCreated, _ = queryResp.GetBoolean(vm_utils.ParamKeyEnable)
			...
			request.SetString(vm_utils.ParamKeyHost, mediaHost)
			request.SetUInt(vm_utils.ParamKeyPort, uint(mediaPort))
			request.SetUInt(vm_utils.ParamKeySize, imageSize)
		}
	}

	{
		...
		request.SetStringArray(vm_utils.ParamKeyAddress, []string{instance.InternalNetwork.AssignedAddress, instance.ExternalNetwork.AssignedAddress})
	}
	var fromSession = request.GetFromSession()
	{
		//redirect request
		request.SetFromSession(id)
		request.SetUIntArray(vm_utils.ParamKeyMode, []uint64{modules.NetworkModePlain, modules.StorageModeLocal})
		request.SetString(vm_utils.ParamKeyInstance, config.ID)
...
func (executor *CreateGuestExecutor) ResponseFail(resp vm_utils.Message, err, target string) error {
	resp.SetSuccess(false)
	resp.SetError(err)
