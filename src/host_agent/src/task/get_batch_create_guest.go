...
func (executor *GetBatchCreateGuestExecutor) Execute(id vm_utils.SessionID, request vm_utils.Message,
	incoming chan vm_utils.Message, terminate chan bool) (err error) {
	var batchID string
	if batchID, err = request.GetString(vm_utils.ParamKeyID); err != nil {
		return err
	}
	resp, _ := vm_utils.CreateJsonMessage(vm_utils.GetBatchCreateGuestResponse)
	...
	resp.SetUIntArray(vm_utils.ParamKeyProgress, createProgress)
	return executor.Sender.SendMessage(resp, request.GetSender())
}
