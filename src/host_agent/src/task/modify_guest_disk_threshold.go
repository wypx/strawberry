...
func (executor *ModifyGuestDiskThresholdExecutor) Execute(id vm_utils.SessionID, request vm_utils.Message,
	...
	resp, _ := vm_utils.CreateJsonMessage(vm_utils.ModifyDiskThresholdResponse)
	...
	{
		//forward request
		forward, _ := vm_utils.CreateJsonMessage(vm_utils.ModifyDiskThresholdRequest)
		forward.SetFromSession(id)
		forward.SetString(vm_utils.ParamKeyGuest, guestID)
		forward.SetUIntArray(vm_utils.ParamKeyLimit, []uint64{readSpeed, writeSpeed, readIOPS, writeIOPS})
