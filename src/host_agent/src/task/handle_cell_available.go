...
func (executor *HandleCellAvailableExecutor) Execute(id vm_utils.SessionID, request vm_utils.Message,
	incoming chan vm_utils.Message, terminate chan bool) error {
	cellName, err := request.GetString(vm_utils.ParamKeyCell)
	if err != nil {
		return err
	}
	cellAddress, err := request.GetString(vm_utils.ParamKeyAddress)
	if err != nil {
		return err
	}
	...
	if cellStatus.PurgeAppending {
		...
		detach.SetStringArray(vm_utils.ParamKeyInstance, []string{})
		...
		case purgeResp := <-incoming:
			if purgeResp.GetID() != vm_utils.DetachInstanceResponse {
	...
	var cellResp vm_utils.Message
	{
		...
		if "" != computePool.Network {
			...
			notify.SetString(vm_utils.ParamKeyMode, addressPool.Provider)
		}
	...
	count, err := cellResp.GetUInt(vm_utils.ParamKeyCount)
	if err != nil {
		return err
	}
	...
	if 0 == count {
		...
		if names, err = cellResp.GetStringArray(vm_utils.ParamKeyName); err != nil {
			return err
		}
		if ids, err = cellResp.GetStringArray(vm_utils.ParamKeyInstance); err != nil {
			return err
		}
		if users, err = cellResp.GetStringArray(vm_utils.ParamKeyUser); err != nil {
			return err
		}
		if groups, err = cellResp.GetStringArray(vm_utils.ParamKeyGroup); err != nil {
			return err
		}
		if secrets, err = cellResp.GetStringArray(vm_utils.ParamKeySecret); err != nil {
			return err
		}
		if addresses, err = cellResp.GetStringArray(vm_utils.ParamKeyAddress); err != nil {
			return err
		}
		if systems, err = cellResp.GetStringArray(vm_utils.ParamKeySystem); err != nil {
			return err
		}
		if internal, err = cellResp.GetStringArray(vm_utils.ParamKeyInternal); err != nil {
			return err
		}
		if external, err = cellResp.GetStringArray(vm_utils.ParamKeyExternal); err != nil {
			return err
		}
		if createTime, err = cellResp.GetStringArray(vm_utils.ParamKeyCreate); err != nil {
			return err
		}
		if hardware, err = cellResp.GetStringArray(vm_utils.ParamKeyHardware); err != nil {
			return err
		}

		if cores, err = cellResp.GetUIntArray(vm_utils.ParamKeyCore); err != nil {
			return err
		}
		if options, err = cellResp.GetUIntArray(vm_utils.ParamKeyOption); err != nil {
			return err
		}
		if enables, err = cellResp.GetUIntArray(vm_utils.ParamKeyEnable); err != nil {
			return err
		}
		if progress, err = cellResp.GetUIntArray(vm_utils.ParamKeyProgress); err != nil {
			return err
		}
		if status, err = cellResp.GetUIntArray(vm_utils.ParamKeyStatus); err != nil {
			return err
		}
		if monitors, err = cellResp.GetUIntArray(vm_utils.ParamKeyMonitor); err != nil {
			return err
		}
		if memories, err = cellResp.GetUIntArray(vm_utils.ParamKeyMemory); err != nil {
			return err
		}
		if diskCounts, err = cellResp.GetUIntArray(vm_utils.ParamKeyCount); err != nil {
			return err
		}
		if disks, err = cellResp.GetUIntArray(vm_utils.ParamKeyDisk); err != nil {
			return err
		}

		if cpuPriorities, err = cellResp.GetUIntArray(vm_utils.ParamKeyPriority); err != nil {
			return err
		}

		if ioLimits, err = cellResp.GetUIntArray(vm_utils.ParamKeyLimit); err != nil {
			return err
		}
